...
<div class="page-main">
	...
	<div class="content-main">
		<h2>구매 목록(관리자 전용)</h2>
		<form action="adminList.do" method="post" id="search_form">
			<ul class="search">
				<li>
					<select name="keyfield" id="keyfield">
						<option value="1" <c:if test="${param.kefield == 1}">selected</c:if>>주문번호</option>
						<option value="2" <c:if test="${param.kefield == 2}">selected</c:if>>ID</option>
						<option value="3" <c:if test="${param.kefield == 3}">selected</c:if>>상품명</option>
					</select>
				</li>
				<li>
