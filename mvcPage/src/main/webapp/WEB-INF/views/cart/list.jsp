			...
			<table>
				...
				<tr>
					...
					<td class="align-center">
						<c:if test="${cart.itemVO.status == 1}">[판매 중지]</c:if>
						<c:if test="${cart.itemVO.quantity < cart.order_quantity}">[수량 부족]<br><small>삭제 후 재고 수량 확인</small></c:if>
						<c:if test="${cart.itemVO.status ==2 and cart.itemVO.quantity >= cart.order_quantity}">
						<input type="number" name="order_quantity" min="1" max="${cart.itemVO.quantity}" autocomplete="off"
					...
					<td class="align-center">
						...
						<br>
						<input type="button" value="삭제" class="cart-del" data-cartnum="${cart.item_num}">
					</td>
				</tr>
				</c:forEach>
				<tr>
					<td colspan="3" class="align-right"><b>총구매금액</b></td>
					<td class="align-center"><fmt:formatNumber value="${all_total}"/>원
				</tr>
			</table>
